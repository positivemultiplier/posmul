...
import { UserId, StudySessionSummary } from "../../../types/study-cycle-types";
  ...
  useEffect(() => {
    if (studyHistory && studyHistory.length > 0) {
      ...
      for (let i = 0; i < studyHistory.length; i++) {
        const sessionDate = new Date(studyHistory[i].completedAt || new Date());
          ...
          studyHistory.slice(0, 5).map((session) => (
            <View key={session.sessionId} style={styles.sessionRow}>
              <View>
                ...
                <Text style={styles.sessionMeta}>{new Date(session.completedAt || new Date()).toLocaleDateString('ko-KR')} • {session.totalTimeMinutes}분 학습</Text>
              </View>
