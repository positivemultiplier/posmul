...
import { StudySessionId } from "../../domain/entities/study-session.entity";
  ...
  const handleStop = () => {
    if (canStop) {
      ...
      if (!result.success) {
        ...
        console.log("Timer stopped. Total time:", result.data, "seconds");
      }
    }
  };
    ...
    <div className={`bg-white rounded-lg shadow-md p-6 ${className}`}>
      ...
      {timerState && (
        <div className="mt-4 text-xs text-gray-500 text-center">
          마지막 저장: {new Date(timerState.lastSaveTime).toLocaleTimeString()}
        </div>
      )}
    </div>
  );
