 ...
 */
export const adaptMCPError = (error: MCPError, operation: string): CompatibleBaseError => {
 ...
 * 레거시 MCP 함수들 (호환성용)
 */
export const mcp_supabase_execute_sql = async (query: string, projectId?: string) => {
  const actualProjectId = projectId || process.env.NEXT_PUBLIC_SUPABASE_PROJECT_ID || 'default';
...
export const createPmcAmount = (amount: number): PmcAmount => amount as PmcAmount;

// 성공/실패 유틸리티 (호환성)
export const success = <T>(data: T): CompatibleResult<T, never> => ({ success: true, data });
export const failure = <E>(error: E): CompatibleResult<never, E> => ({ success: false, error });
export const isSuccess = <T, E>(result: CompatibleResult<T, E>): result is { success: true; data: T } => result.success;
export const isFailure = <T, E>(result: CompatibleResult<T, E>): result is { success: false; error: E } => !result.success;

// 호환성 헬퍼들
export const adaptErrorToBaseError = (error: unknown, operation: string): CompatibleBaseError => {
