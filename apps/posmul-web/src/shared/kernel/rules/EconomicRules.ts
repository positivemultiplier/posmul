...
export class EconomicRules {
  ...
  static calculatePmpReward(
    ...
    const moneyWaveMultiplier = this.getMoneyWaveMultiplier(
      context.currentMoneyWave
    );
  ...
  static validatePmcSpending(
    ...
    if (amount < minSpend) {
      return {
        valid: false,
        reason: `Minimum spend for ${purpose} is ${minSpend} PmcAmount`,
      };
    }
    ...
    if (amount > maxSpend) {
      return {
        valid: false,
        reason: `Maximum spend for ${purpose} is ${maxSpend} PmcAmount`,
      };
    }
  ...
  private static getMoneyWaveMultiplier(moneyWave?: {
    multiplier: number;
  }): number {
  ...
  private static getMaximumSpend(
    purpose: SpendPurpose,
    userLevel: number
  ): number {
