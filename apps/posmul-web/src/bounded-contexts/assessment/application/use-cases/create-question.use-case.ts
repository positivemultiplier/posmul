...
import { Assessment, Question, AssessmentId } from "../../domain/entities/assessment.entity";
import { Result, success, failure, UseCaseError, NotFoundError, DomainError } from "@/shared/errors";
  ...
  async execute(
    ...
    try {
      ...
      if (!saveResult.success) {
        return failure(new UseCaseError("Failed to save assessment.", saveResult.error));
      }

      return success(question);
    } catch (error: unknown) {
      return failure(new UseCaseError("An unexpected error occurred while creating the question.", error as Error));
    }
  }
} 