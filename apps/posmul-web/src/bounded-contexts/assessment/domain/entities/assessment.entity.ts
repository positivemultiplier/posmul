...
import { Result, success, failure, ValidationError, DomainError } from "@/shared/errors";
import { SolutionTemplate, SolutionTemplateId } from "./solution-template.entity";
...
export class Question extends BaseEntity<IQuestionProps> {
  ...
  public static create(
    ...
    if (data.points <= 0) {
      return failure(new ValidationError("점수는 0보다 커야 합니다.", "points"));
    }

    if (data.type === QuestionType.MultipleChoice) {
      if (!data.options || data.options.length < 2) {
        return failure(new ValidationError("객관식 문제는 최소 2개의 선택지가 필요합니다.", "options"));
      }
      const correctOptions = data.options.filter(opt => opt.isCorrect);
      if (correctOptions.length === 0) {
        return failure(new ValidationError("정답이 설정되지 않았습니다.", "options"));
      }
    }
  ...
  get id(): QuestionId { return this._id; }
  get templateId(): SolutionTemplateId { return this.props.templateId; }
  get type(): QuestionType { return this.props.type; }
  get title(): string { return this.props.title; }
  get content(): string { return this.props.content; }
  get points(): number { return this.props.points; }
  get options(): QuestionOption[] { return this.props.options ? [...this.props.options] : []; }
  get correctAnswer(): string | undefined { return this.props.correctAnswer; }
  get difficulty(): string { return this.props.difficulty; }
  get tags(): string[] { return [...this.props.tags]; }
  get isActive(): boolean { return this.props.isActive; }
  get gradingCriteria(): string | undefined { return this.props.gradingCriteria; }
  get timeLimit(): number | undefined { return this.props.timeLimit; }
  get createdAt(): Date { return this.props.createdAt; }

  public gradeSubmission(answer: string | string[]): Result<number, DomainError> {
    if (!this.props.isActive) {
      return failure(new DomainError("비활성화된 문제는 채점할 수 없습니다.", "QUESTION_INACTIVE"));
    }
    ...
    switch (this.props.type) {
      ...
      default:
        return failure(new DomainError("자동 채점이 지원되지 않는 문제 유형입니다.", "UNSUPPORTED_QUESTION_TYPE"));
    }
  }

  private gradeMultipleChoice(answer: string): Result<number, DomainError> {
    if (!this.props.options) {
      return failure(new DomainError("선택지가 설정되지 않았습니다.", "NO_OPTIONS"));
    }

    const selectedOption = this.props.options.find(opt => opt.id === answer);
    if (!selectedOption) {
      return failure(new DomainError("유효하지 않은 선택지입니다.", "INVALID_OPTION"));
    }
  ...
  private gradeShortAnswer(answer: string): Result<number, DomainError> {
    if (!this.props.correctAnswer) {
      return failure(new DomainError("정답이 설정되지 않았습니다.", "NO_CORRECT_ANSWER"));
    }
...
export class Submission extends BaseEntity<ISubmissionProps> {
  ...
  public static create(
    ...
    if (data.finalScore < 0) {
      return failure(new ValidationError("최종 점수는 0보다 작을 수 없습니다.", "finalScore"));
    }
  ...
  public static reconstitute(props: ISubmissionProps, id: SubmissionId): Submission {
    return new Submission(props, id);
  }

  get id(): SubmissionId { return this._id; }
  get assessmentId(): AssessmentId { return this.props.assessmentId; }
  get studentId(): string { return this.props.studentId; }
  get answers(): IGradedAnswer[] { return this.props.answers; }
  ...
  get timeSpent(): number | undefined { return this.props.timeSpent; }
  get createdAt(): Date { return this.props.createdAt; }
...
export class Assessment extends BaseEntity<IAssessmentProps> {
  ...
  public static create(
    data: Omit<IAssessmentProps, "questions" | "totalPoints" | "status" | "createdAt" | "updatedAt"> & {
      status?: AssessmentStatus;
    }
    ...
    if (!data.creatorId) {
      return failure(new ValidationError("생성자 ID는 필수입니다.", "creatorId"));
    }
    if (data.allowedAttempts < 1) {
      return failure(new ValidationError("허용 시도 횟수는 1 이상이어야 합니다.", "allowedAttempts"));
    }
    if (data.passingScore < 0 || data.passingScore > 100) {
      return failure(new ValidationError("합격 점수는 0-100 사이여야 합니다.", "passingScore"));
    }
  ...
  public static reconstitute(props: IAssessmentProps, id: AssessmentId): Assessment {
  ...
  get questions(): Question[] { return [...this.props.questions]; }
  get totalPoints(): number { return this.props.totalPoints; }
  get allowedAttempts(): number { return this.props.allowedAttempts; }
  get passingScore(): number { return this.props.passingScore; }
  get timeLimit(): number | undefined { return this.props.timeLimit; }
  get startDate(): Date | undefined { return this.props.startDate; }
  get endDate(): Date | undefined { return this.props.endDate; }
  get isRandomized(): boolean { return this.props.isRandomized; }
  get showResults(): boolean { return this.props.showResults; }
  get settings(): AssessmentSettings { return this.props.settings; }
  get createdAt(): Date { return this.props.createdAt; }

  public addQuestion(question: Question): Result<void, DomainError> {
    if (this.props.status !== AssessmentStatus.Draft) {
      return failure(new DomainError("초안 상태에서만 문제를 추가할 수 있습니다.", "ASSESSMENT_NOT_DRAFT"));
    }
  ...
  public publish(): Result<void, DomainError> {
    if (this.props.status !== AssessmentStatus.Draft) {
      return failure(new DomainError("초안 상태에서만 발행할 수 있습니다.", "INVALID_STATUS"));
    }

    if (this.props.questions.length === 0) {
      return failure(new DomainError("최소 1개의 문제가 필요합니다.", "NO_QUESTIONS"));
    }
  ...
  public gradeSubmissions(submissions: Submission[]): Result<AssessmentResult, DomainError> {
    ...
    */
   return failure(new DomainError("Not implemented", "GRADE_SUBMISSIONS_NOT_IMPLEMENTED"));
  }

  private recalculateTotalPoints(): void {
    this.props.totalPoints = this.props.questions.reduce((sum, q) => sum + q.points, 0);
  }
} 