...
export class Submission extends BaseEntity<ISubmissionProps> {
  ...
  public finalize(): Result<void, Error> {
    ...
    this.props.timeSpent = (this.props.submittedAt.getTime() - this.props.startedAt.getTime()) / 1000;
    return success(undefined);
  }

  public grade(gradedAnswers: Pick<SubmittedAnswer, 'questionId' | 'isCorrect' | 'score'>[]): void {
    let totalScore = 0;
    this.props.answers.forEach(answer => {
      const gradeInfo = gradedAnswers.find(g => g.questionId === answer.questionId);
      if(gradeInfo) {
