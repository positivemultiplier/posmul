...
import { AssessmentId, createAssessmentId } from "../value-objects/assessment-id.value-object";
...
export class Assessment extends BaseEntity<IAssessmentProps> {
  ...
  public static create(props: IAssessmentProps): Result<Assessment, Error> {
    ...
    if (props.questions.length > 50) {
      return failure(new Error('Cannot create an assessment with more than 50 questions.'));
    }

    const totalPointsFromQuestions = props.questions.reduce((total, question) => total + question.props.points, 0);
  ...
  public removeQuestion(questionId: QuestionId): void {
    const questionToRemove = this.props.questions.find(q => (q as any).id.equals(questionId));
    if (questionToRemove) {
      this.props.totalPoints -= questionToRemove.props.points;
      this.props.questions = this.props.questions.filter(q => !(q as any).id.equals(questionId));
      this.props.updatedAt = new Date();
    }
  }
  ...
  public publish(): Result<void, Error> {
    if (this.props.questions.length === 0) {
      return failure(new Error('Cannot publish an assessment with no questions.'));
    }
