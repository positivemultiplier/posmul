...
export function useAuth(): AuthState & AuthActions {
  ...
  const signIn = async (credentials: { email: string; password: string }) => {
    try {
      ...
      // SDK를 통한 로그인
      const result = await authClient.auth.signIn(credentials.email as any, credentials.password);
      ...
      if (result.success) {
        ...
        const errorMessage = 'error' in result && result.error?.message ? result.error.message : '로그인에 실패했습니다.';
  ...
  const signUp = async (data: { email: string; password: string; displayName?: string }) => {
    try {
      ...
      if (result.success) {
        ...
        const errorMessage = 'error' in result && result.error?.message ? result.error.message : '회원가입에 실패했습니다.';
        throw new Error(errorMessage);
      }
  ...
  const signOut = async () => {
    try {
      ...
      if (result.success) {
        ...
        const errorMessage = 'error' in result && result.error?.message ? result.error.message : '로그아웃에 실패했습니다.';
        throw new Error(errorMessage);
      }
  ...
  const refreshToken = async () => {
    try {
      ...
      if (result.success) {
        ...
        const errorMessage = 'error' in result && result.error?.message ? result.error.message : '토큰 갱신에 실패했습니다.';
