...
export class Investment {
  ...
  static create(
    ...
    // 검증
    if (amount < 0) {
      return { success: false, error: new Error('Investment amount cannot be negative') };
    }

    // 타입별 최소 금액 검증
    if (type === InvestmentType.CLOUD_FUNDING && amount < 1000) {
      return { success: false, error: new Error('Minimum cloud funding amount is 1,000 KRW') };
    }

    if (type === InvestmentType.LOCAL_LEAGUE && amount < 1000) {
      return { success: false, error: new Error('Minimum local league payment is 1,000 KRW') };
    }
    ...
    // 도메인 이벤트 발행
    investment.addDomainEvent(new InvestmentCreatedEvent(
      id.getValue(),
      userId,
      type,
      targetId,
  ...
  completeLocalLeaguePayment(pmcRate: number = 0.01): Result<void> {
    ...
    // 도메인 이벤트 발행
    this.addDomainEvent(new InvestmentCompletedEvent(
      this.id.getValue(),
      this.pmcEarned,
      this.pmpEarned
    ));
  ...
  completeMajorLeagueViewing(pmcEarned: number): Result<void> {
    if (this.type !== InvestmentType.MAJOR_LEAGUE) {
      return { success: false, error: new Error('Not a major league activity') };
    }
  ...
  completeCloudFundingInvestment(): Result<void> {
    if (this.type !== InvestmentType.CLOUD_FUNDING) {
      return { success: false, error: new Error('Not a cloud funding investment') };
    }
  ...
  cancel(): Result<void> {
    if (this.status !== InvestmentStatus.PENDING) {
      return { success: false, error: new Error('Can only cancel pending investments') };
    }
  ...
  refund(reason: string): Result<void> {
    if (this.status !== InvestmentStatus.COMPLETED) {
      return { success: false, error: new Error('Can only refund completed investments') };
    }

    if (this.type === InvestmentType.LOCAL_LEAGUE || this.type === InvestmentType.MAJOR_LEAGUE) {
      return { success: false, error: new Error('Local League payments and Major League activities cannot be refunded') };
    }
    ...
    // 도메인 이벤트 발행
    this.addDomainEvent(new InvestmentRefundedEvent(
      this.id.getValue(),
      this.amount,
      reason
    ));
  ...
  getInvestmentSummary(): {
    ...
    const daysSinceCompleted = this.completedAt 
      ? Math.floor((now.getTime() - this.completedAt.getTime()) / (1000 * 60 * 60 * 24))
      : undefined;
