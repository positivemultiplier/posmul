...
import { InvestmentType, MerchantId, AdvertisementId, CrowdFundingId } from '../../domain/value-objects/investment-value-objects';
import { CreateInvestmentRequest, CreateInvestmentRequestSchema } from '../dto/investment.dto';
...
export class CreateInvestmentUseCase {
  ...
  async execute(
    ...
    try {
      ...
      switch (validData.type) {
        case InvestmentType.LOCAL_LEAGUE:
          ...
          if (!merchantResult.success || !merchantResult.data) {
            return { success: false, error: new Error('투자 대상 상점을 찾을 수 없습니다.') };
          }
        ...
        case InvestmentType.MAJOR_LEAGUE:
          ...
          if (!advertisementResult.success || !advertisementResult.data) {
            return { success: false, error: new Error('투자 대상 광고를 찾을 수 없습니다.') };
          }
        ...
        case InvestmentType.CLOUD_FUNDING:
          ...
          if (!crowdFundingResult.success || !crowdFundingResult.data) {
            return { success: false, error: new Error('투자 대상 크라우드 펀딩을 찾을 수 없습니다.') };
          }
          target = crowdFundingResult.data;
          break;

        default:
          return { success: false, error: new Error('지원하지 않는 투자 타입입니다.') };
      }
      ...
      // 도메인 서비스를 통한 투자 검증
      const validationResult2 = InvestmentDomainService.validateInvestmentEligibility(
      ...
      // 예상 수익 계산
      const estimatedReturnResult = InvestmentDomainService.calculateInvestmentReward(
        investmentResult.data,
      ...
      return {
        success: false,
        error: error instanceof Error ? error : new Error('투자 생성 중 오류가 발생했습니다.')
      };
    }
  }
}
