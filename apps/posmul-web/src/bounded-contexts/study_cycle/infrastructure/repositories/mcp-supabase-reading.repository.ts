...
import { Reading, ReadingId, ChapterProgress } from "../../domain/entities/reading.entity";
...
type ReadingProgressRow = Tables<{ schema: "study_cycle" }, "sc_reading_progress">;
...
function handleRepositoryError(error: unknown, operation: string): RepositoryError {
...
export class McpSupabaseReadingRepository implements IReadingRepository {
  ...
  async save(reading: Reading): Promise<Result<void, RepositoryError>> {
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to save reading: ${error.message}`));
      }

      // Save chapter progress if any
      if (reading.chaptersProgress.length > 0) {
        const saveProgressResult = await this.saveChapterProgress(reading.id, reading.chaptersProgress);
  ...
  async findById(id: ReadingId): Promise<Result<Reading | null, RepositoryError>> {
    try {
      const query = `
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find reading by ID: ${error.message}`));
      }
  ...
  async findByUserAndTextbook(userId: UserId, textbookId: TextbookId): Promise<Result<Reading[], RepositoryError>> {
    try {
      const query = `
        SELECT * FROM study_cycle.sc_readings 
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find readings by user and textbook: ${error.message}`));
      }

      const readings = await Promise.all(
        (data || []).map(async (row) => {
          const readingRow = row as ReadingRow;
          const chaptersProgressResult = await this.loadChapterProgress(readingRow.id as ReadingId);
          
          return Reading.fromPersistence(readingRow, {
            chaptersProgress: chaptersProgressResult.success ? chaptersProgressResult.data : [],
            studySessionIds: [],
          });
        })
      );
      ...
      return failure(handleRepositoryError(error, "find readings by user and textbook"));
    }
  }

  async findLatestByUserAndTextbook(userId: UserId, textbookId: TextbookId): Promise<Result<Reading | null, RepositoryError>> {
    try {
      const query = `
        SELECT * FROM study_cycle.sc_readings 
        WHERE user_id = '${userId}' AND textbook_id = '${textbookId}'
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find latest reading: ${error.message}`));
      }
  ...
  async findByUserId(userId: UserId): Promise<Result<Reading[], RepositoryError>> {
    try {
      const query = `
        SELECT * FROM study_cycle.sc_readings 
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find readings by user: ${error.message}`));
      }
  ...
  async delete(id: ReadingId): Promise<Result<void, RepositoryError>> {
    try {
      ...
      if (progressError) {
        return failure(new RepositoryError(`Failed to delete reading progress: ${progressError.message}`));
      }
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to delete reading: ${error.message}`));
      }
  ...
  private async loadChapterProgress(readingId: ReadingId): Promise<Result<ChapterProgress[], RepositoryError>> {
    try {
      const query = `
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to load chapter progress: ${error.message}`));
      }

      const chaptersProgress: ChapterProgress[] = (data || []).map(row => {
        const progressRow = row as ReadingProgressRow & { chapter_title: string; chapter_order: number };
        return {
          chapterId: progressRow.chapter_id,
          chapterTitle: progressRow.chapter_title || `Chapter ${progressRow.chapter_order || 1}`,
          totalPages: 100, // TODO: Get from chapter data when available
          completedPages: progressRow.pages_read || 0,
          lastReadAt: progressRow.updated_at ? new Date(progressRow.updated_at) : undefined,
  ...
  private async saveChapterProgress(readingId: ReadingId, chaptersProgress: ChapterProgress[]): Promise<Result<void, RepositoryError>> {
    try {
      ...
      if (deleteError) {
        return failure(new RepositoryError(`Failed to clear existing progress: ${deleteError.message}`));
      }
      ...
      for (const progress of chaptersProgress) {
        ...
        if (error) {
          return failure(new RepositoryError(`Failed to save chapter progress: ${error.message}`));
        }
      }
