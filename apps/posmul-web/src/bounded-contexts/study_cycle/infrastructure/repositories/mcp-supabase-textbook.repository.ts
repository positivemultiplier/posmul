...
declare const mcp_supabase_execute_sql: (params: {
  project_id: string;
  query: string;
}) => Promise<{ data: Record<string, unknown>[] | null; error: Record<string, unknown> | null }>;
...
export class McpSupabaseTextbookRepository implements ITextbookRepository {
    ...
    async save(textbook: Textbook): Promise<Result<void, RepositoryError>> {
        ...
        try {
            ...
            if (error) return failure(new RepositoryError(`Failed to save textbook: ${JSON.stringify(error)}`));
            return success(undefined);
        } catch (e) {
            return failure(new RepositoryError("Failed to save textbook", e as Error));
        }
    }

    async findById(id: TextbookId): Promise<Result<Textbook | null, RepositoryError>> {
        ...
        try {
            ...
            if (error) return failure(new RepositoryError(`Failed to find textbook by id: ${JSON.stringify(error)}`));
            if (!data || data.length === 0) {
                return success(null);
            }
            return success(this.toDomain(data[0]));
        } catch (e) {
            return failure(new RepositoryError("Failed to find textbook by id", e as Error));
        }
    }
    ...
    async delete(id: TextbookId): Promise<Result<void, RepositoryError>> {
        ...
        try {
            ...
            if (error) return failure(new RepositoryError(`Failed to delete textbook: ${JSON.stringify(error)}`));
            return success(undefined);
        } catch (e) {
            return failure(new RepositoryError("Failed to delete textbook", e as Error));
        }
    }

    async findAllByCreator(creatorId: string): Promise<Result<Textbook[], RepositoryError>> {
        ...
        try {
            ...
            if (error) return failure(new RepositoryError(`Failed to find textbooks by creator: ${JSON.stringify(error)}`));
            if (!data) return success([]);

            const textbooks = data.map(row => this.toDomain(row));
            return success(textbooks);
        } catch(e) {
            return failure(new RepositoryError("Failed to find textbooks by creator", e as Error));
        }
    }
} 