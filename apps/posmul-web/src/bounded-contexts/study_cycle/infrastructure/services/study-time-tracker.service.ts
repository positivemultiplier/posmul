...
export class StudyTimeTrackerService {
  ...
  public startTimer(sessionId: StudySessionId): Result<void, DomainError> {
    try {
      if (this.timerState?.status === StudySessionStatus.ACTIVE) {
        return failure(new DomainError("Timer is already running", "TIMER_ALREADY_ACTIVE"));
      }
  ...
  public pauseTimer(reason: 'manual' | 'visibility' | 'beforeunload' = 'manual'): Result<void, DomainError> {
    try {
      if (!this.timerState || this.timerState.status !== StudySessionStatus.ACTIVE) {
        return failure(new DomainError("No active timer to pause", "NO_ACTIVE_TIMER"));
      }
  ...
  public resumeTimer(reason: 'manual' | 'visibility' = 'manual'): Result<void, DomainError> {
    try {
      if (!this.timerState || this.timerState.status !== StudySessionStatus.PAUSED) {
        return failure(new DomainError("No paused timer to resume", "NO_PAUSED_TIMER"));
      }
  ...
  public restoreFromStorage(): Result<TimerState | null, DomainError> {
    try {
      ...
      const now = new Date();
      const timeSinceLastSave = Math.floor((now.getTime() - savedState.lastSaveTime.getTime()) / 1000);
      ...
      return failure(new DomainError(
        error instanceof Error ? error.message : "Failed to restore timer state",
        "TIMER_RESTORE_ERROR"
      ));
    }
  }
    ...
    this.autoSaveIntervalId = setInterval(async () => {
      if (this.timerState && this.callbacks.onAutoSave) {
        try {
          ...
          this.callbacks.onError?.(error instanceof Error ? error : new Error("Auto save failed"));
        }
      }
  ...
  private saveStateToStorage(): void {
    ...
    try {
      ...
      this.callbacks.onError?.(new Error("Failed to save timer state to storage"));
    }
  }
  ...
  private loadStateFromStorage(): TimerState | null {
    ...
    try {
      ...
      this.callbacks.onError?.(new Error("Failed to load timer state from storage"));
      return null;
    }
  }
  ...
  private clearStateFromStorage(): void {
    ...
    try {
      ...
      this.callbacks.onError?.(new Error("Failed to clear timer state from storage"));
    }
  }
