...
import { RecordProgressRequest, AcknowledgeResponse } from '../dto/study-session.dto';
import { Result, success, failure, IUseCase } from '@/shared/types';
import { UseCaseError } from '@/shared/errors';

export class RecordProgressUseCase implements IUseCase<RecordProgressRequest, AcknowledgeResponse> {
  constructor(private readonly studySessionRepository: IStudySessionRepository) {}

  async execute(request: RecordProgressRequest): Promise<Result<AcknowledgeResponse, UseCaseError>> {
    try {
      // 1. Find the study session by ID
      const sessionResult = await this.studySessionRepository.findById(request.sessionId);
      if (!sessionResult.success) {
        return failure(new UseCaseError('Failed to retrieve study session', sessionResult.error));
      }
      ...
      if (session.userId !== request.userId) {
        return failure(new UseCaseError('User is not authorized to record progress for this session.'));
      }
      ...
      if (!progressResult.success) {
        return failure(new UseCaseError(progressResult.error.message, progressResult.error));
      }

      // 4. Save the updated session
      const saveResult = await this.studySessionRepository.save(session);
      if (!saveResult.success) {
        return failure(new UseCaseError('Failed to save session with new progress', saveResult.error));
      }

      // 5. Return acknowledgement
      return success({ success: true, message: 'Progress recorded successfully.' });
    } catch (error) {
