...
export class Advertisement {
  ...
  static create(
    ...
    if (!contentUrl || !thumbnailUrl) {
      return {
        success: false,
        error: new Error("Content URL and thumbnail URL are required"),
      };
    }
    ...
    if (targetAudience.length === 0) {
      return {
        success: false,
        error: new Error("Target audience must be specified"),
      };
    }
  ...
  publish(): Result<void> {
    if (this.status !== AdvertisementStatus.DRAFT) {
      return {
        success: false,
        error: new Error("Only draft advertisements can be published"),
      };
    }
  ...
  startViewing(viewerId: UserId): Result<void> {
    if (this.status !== AdvertisementStatus.PUBLISHED) {
      return {
        success: false,
        error: new Error("Advertisement is not published"),
      };
    }
    ...
    if (existingRecord) {
      return {
        success: false,
        error: new Error("User is already viewing this advertisement"),
      };
    }
  ...
  completeViewing(
    ...
    if (completed) {
      this.addDomainEvent(
        new AdvertisementCompletedEvent(
          this.id.getValue(),
          viewerId,
  ...
  deactivate(): Result<void> {
    if (this.status === AdvertisementStatus.ARCHIVED) {
      return {
        success: false,
        error: new Error("Advertisement is already archived"),
      };
    }
  ...
  getCompletionRate(): number {
    ...
    const completedViews = this.viewingRecords.filter(
  ...
  getAverageRating(): number {
    ...
    const totalRating = ratingsExist.reduce(
  ...
  getTotalPmcDistributed(): number {
    return this.viewingRecords.reduce(
      (total, record) => total + record.pmcEarned,
      0
    );
  }
  ...
  getStatistics(): {
    ...
    const totalDuration = this.viewingRecords.reduce(
      (sum, record) => sum + record.duration.getSeconds(),
      0
    );
