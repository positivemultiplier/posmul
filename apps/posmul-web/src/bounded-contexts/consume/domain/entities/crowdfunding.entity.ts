...
export class CrowdFunding {
  ...
  static create(
    ...
    if (endDate <= new Date()) {
      return {
        success: false,
        error: new Error("End date must be in the future"),
      };
    }
    ...
    // 도메인 이벤트 발행
    crowdFunding.addDomainEvent(
      new CrowdFundingCreatedEvent(
        id.getValue(),
        title.trim(),
        category,
  ...
  launch(): Result<void> {
    if (this.status !== FundingStatus.DRAFT) {
      return {
        success: false,
        error: new Error("Only draft fundings can be launched"),
      };
    }
    ...
    if (new Date() < this.startDate) {
      return {
        success: false,
        error: new Error("Cannot launch before start date"),
      };
    }
  ...
  invest(investorId: UserId, amount: number, message?: string): Result<void> {
    ...
    if (amount < 1000) {
      return {
        success: false,
        error: new Error("Minimum investment amount is 1,000 KRW"),
      };
    }
    ...
    if (amount % 1000 !== 0) {
      return {
        success: false,
        error: new Error("Investment amount must be in 1,000 KRW units"),
      };
    }
    ...
    if (currentAmount + amount > this.goal.getTargetAmount()) {
      return {
        success: false,
        error: new Error("Investment would exceed funding goal"),
      };
    }
    ...
    // 도메인 이벤트 발행
    this.addDomainEvent(
      new CrowdFundingInvestmentEvent(
        this.id.getValue(),
        investorId,
        amount,
  ...
  private completeSuccess(): void {
    ...
    this.addDomainEvent(
      new CrowdFundingCompletedEvent(
        this.id.getValue(),
        this.getCurrentAmount(),
  ...
  completeFailed(): Result<void> {
    if (this.status !== FundingStatus.ACTIVE) {
      return {
        success: false,
        error: new Error("Only active fundings can be marked as failed"),
      };
    }
    ...
    if (new Date() <= this.endDate) {
      return {
        success: false,
        error: new Error("Cannot mark as failed before end date"),
      };
    }
  ...
  cancel(): Result<void> {
    if (
      ...
      return {
        success: false,
        error: new Error("Cannot cancel completed funding"),
      };
    }
  ...
  addUpdate(
    ...
    if (
      ...
      return {
        success: false,
        error: new Error(
          "Can only add updates to active or successful fundings"
        ),
      };
    }
    ...
    if (!title || title.trim().length === 0) {
      return {
        success: false,
        error: new Error("Update title cannot be empty"),
      };
    }
    ...
    if (!content || content.trim().length === 0) {
      return {
        success: false,
        error: new Error("Update content cannot be empty"),
      };
    }
