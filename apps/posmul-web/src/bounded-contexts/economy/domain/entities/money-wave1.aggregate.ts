/**
 * MoneyWave1 Aggregate - EBIT 기반 PmcAmount 발행 시스템
 *
 * Jensen & Meckling Agency Theory를 적용하여
 * 기업의 EBIT(Earnings Before Interest and Taxes) 데이터를 기반으로
 * PmcAmount를 자동 발행하는 메커니즘을 구현합니다.
 *
 * 핵심 불변성:
 * - EBIT가 양수인 경우에만 PmcAmount 발행 가능
...
export interface PmcAmountEmissionPolicy {
  readonly maxDailyEmission: PmcAmount;
...
export interface EmissionEvent {
  ...
  readonly emissionAmount: PmcAmount;
...
export class MoneyWave1Aggregate {
  private constructor(
    ...
    private emissionPolicy: PmcAmountEmissionPolicy,
    private totalEmitted: PmcAmount,
  ...
  static create(
    id: MoneyWaveId,
    initialPolicy: Omit<PmcAmountEmissionPolicy, "maxDailyEmission">
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          error: new DomainError("INVALID_AGENCY_THRESHOLD"),
        };
      }

      const defaultPolicy: PmcAmountEmissionPolicy = {
        ...initialPolicy,
        maxDailyEmission: createPmcAmount(0), // EBIT 계산 후 설정
      };
      ...
      return {
        ...
        error: new DomainError(
          ...
          { code: "MONEY_WAVE1_CREATION_FAILED" }
        ),
      };
    }
  }
  ...
  /**
   * EBIT 데이터를 기반으로 PmcAmount 발행 정책을 설정합니다.
  ...
  setEBITCalculation(ebitData: EBITCalculation): Result<void> {
    try {
      ...
      if (ebit <= 0) {
        return {
          success: false,
          error: new DomainError("NEGATIVE_EBIT"),
        };
      }
      ...
      if (ebitData.validUntil <= new Date()) {
        return {
          success: false,
          error: new DomainError("EXPIRED_EBIT_DATA,"),
        };
      }

      // 최대 일일 발행량 계산 (연간 순이익 추정치의 1/365)
      const annualNetIncome = ebit * 0.75; // 세금 및 이자 고려 (25% 가정)
      const maxDailyEmission = createPmcAmount(
        Math.max(0, annualNetIncome / 365)
      );
   ...
   * 높은 점수일수록 정보 비대칭이 해소되어 더 많은 PmcAmount 발행이 가능합니다.
   */
  updateAgencyScore(
    ...
    try {
      ...
      if (inputs.some((val) => val < 0 || val > 1)) {
        return {
          success: false,
          error: new DomainError("INVALID_AGENCY_METRICS,"),
        };
      }
  ...
  /**
   * 실제 PmcAmount 발행을 실행합니다.
   * 비즈니스 불변성을 엄격히 검증합니다.
   */
  emitPmcAmount(
    requestedAmount: PmcAmount,
    reason: string
  ): Result<EmissionEvent> {
    try {
      // 전제 조건 검증
      if (
        this.status !== MoneyWave1Status.EMISSION_READY &&
        ...
        return {
          success: false,
          error: new DomainError("INVALID_EMISSION_STATE"),
        };
      }
      ...
      if (!this.ebitCalculation) {
        return {
          success: false,
          error: new DomainError("MISSING_EBIT_DATA,"),
        };
      }
      ...
      // 발행량 제한 검증
      const dailyEmissionCap = unwrapPmcAmount(
        this.emissionPolicy.maxDailyEmission
      );
      const requestedAmountRaw = unwrapPmcAmount(requestedAmount);
      ...
      const actualEmissionAmount = createPmcAmount(
      ...
      // 상태 업데이트
      this.totalEmitted = createPmcAmount(
        unwrapPmcAmount(this.totalEmitted) +
          unwrapPmcAmount(actualEmissionAmount)
      );
  ...
  updateEmissionPolicy(
    newPolicy: Partial<PmcAmountEmissionPolicy>
  ): Result<void> {
    try {
      if (
  ...
  getEmissionPolicy(): PmcAmountEmissionPolicy {
    return { ...this.emissionPolicy };
  }

  getTotalEmitted(): PmcAmount {
   ...
   */
  getAvailableEmissionAmount(): PmcAmount {
    if (
      !this.ebitCalculation ||
      this.status !== MoneyWave1Status.EMISSION_READY
    ) {
      return createPmcAmount(0);
    }

    const maxDaily = unwrapPmcAmount(this.emissionPolicy.maxDailyEmission);
    const agencyAdjustment =
      this.agencyScore * this.emissionPolicy.riskAdjustmentFactor;

    return createPmcAmount(maxDaily * agencyAdjustment);
  }
  ...
  validateInvariants(): Result<void> {
    ...
    // 2. 발행량 제한 준수
    const totalEmittedAmount = unwrapPmcAmount(this.totalEmitted);
