...
} from "../components/realtime/notification-system";
...
} from "../components/realtime/realtime-charts";
...
export const realtimeUtils = {
   ...
   * 통화 형식을 포맷팅합니다
   */
  formatCurrency: (amount: number, currency: "PmpAmount" | "PmcAmount" | "KRW" = "KRW") => {
    const formatted = amount.toLocaleString("ko-KR");
    switch (currency) {
      case "PmpAmount":
        return `${formatted} PmpAmount`;
      case "PmcAmount":
        return `${formatted} PmcAmount`;
      default:
        return `₩${formatted}`;
    }
  },
