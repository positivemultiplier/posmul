...
import type { Result as ResultType } from '../types/common';

// 결과 패턴 헬퍼 함수들
export const ResultUtils = {
  success: <T>(data: T): ResultType<T> => ({ success: true, data }),
  failure: <T>(error: Error): ResultType<T> => ({ success: false, error }),
  
  isSuccess: <T>(result: ResultType<T>): result is { success: true; data: T } =>
    result.success,
  
  isFailure: <T>(result: ResultType<T>): result is { success: false; error: Error } =>
    !result.success,
  ...
  map: <T, U>(
    ...
    return ResultUtils.failure(result.error);
  },
...
export const DateUtils = {
  ...
  isAfter: (date1: Date, date2: Date): boolean => date1.getTime() > date2.getTime(),
  
  isBefore: (date1: Date, date2: Date): boolean => date1.getTime() < date2.getTime(),
