...
export class GetTextbookUseCase implements IUseCase<TextbookId, TextbookResponseDto | null> {
    ...
    async execute(
        ...
        if (!textbookResult.success) {
            return { success: false, error: new UseCaseError("Failed to get textbook", textbookResult.error) };
        }
