...
import { StudySession, StudySessionId, UserId, StudySessionStatus } from "../../domain/entities/study-session.entity";
...
export class McpSupabaseStudySessionRepository implements IStudySessionRepository {
  ...
  async save(session: StudySession): Promise<Result<void, RepositoryError>> {
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to save study session: ${error.message}`));
      }
  ...
  async findById(id: StudySessionId): Promise<Result<StudySession | null, RepositoryError>> {
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find study session by ID: ${error.message}`));
      }
  ...
  async findActiveByUserId(userId: UserId): Promise<Result<StudySession | null, RepositoryError>> {
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find active study session: ${error.message}`));
      }
      ...
      const session = StudySession.fromPersistence(data[0] as StudySessionRow, { status: StudySessionStatus.ACTIVE });
      return success(session);
    } catch (error) {
  ...
  async findByUserId(userId: UserId, limit = 50, offset = 0): Promise<Result<StudySession[], RepositoryError>> {
    try {
      const query = `
        SELECT * FROM study_cycle.sc_study_sessions 
        WHERE user_id = '${userId}'
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find study sessions by user: ${error.message}`));
      }

      const sessions = (data || []).map(row => StudySession.fromPersistence(row as StudySessionRow));
      return success(sessions);
    } catch (error) {
      return failure(handleRepositoryError(error, "find study sessions by user"));
    }
  }

  async findByTextbookId(textbookId: TextbookId, limit = 50, offset = 0): Promise<Result<StudySession[], RepositoryError>> {
    try {
      const query = `
        SELECT * FROM study_cycle.sc_study_sessions 
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find study sessions by textbook: ${error.message}`));
      }

      const sessions = (data || []).map(row => StudySession.fromPersistence(row as StudySessionRow));
      return success(sessions);
    } catch (error) {
      return failure(handleRepositoryError(error, "find study sessions by textbook"));
    }
  }

  async findByUserAndTextbook(userId: UserId, textbookId: TextbookId): Promise<Result<StudySession[], RepositoryError>> {
    try {
      const query = `
        SELECT * FROM study_cycle.sc_study_sessions 
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to find study sessions by user and textbook: ${error.message}`));
      }

      const sessions = (data || []).map(row => StudySession.fromPersistence(row as StudySessionRow));
      return success(sessions);
    } catch (error) {
      return failure(handleRepositoryError(error, "find study sessions by user and textbook"));
    }
  }
  ...
  async delete(id: StudySessionId): Promise<Result<void, RepositoryError>> {
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to delete study session: ${error.message}`));
      }
  ...
  async getTotalStudyTimeByUserId(userId: UserId): Promise<Result<number, RepositoryError>> {
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to get total study time: ${error.message}`));
      }
  ...
  async getStudyStatsByUserId(userId: UserId): Promise<Result<{
    ...
    try {
      ...
      if (error) {
        return failure(new RepositoryError(`Failed to get study stats: ${error.message}`));
      }
