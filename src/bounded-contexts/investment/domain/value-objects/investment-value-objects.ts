...
export class AdvertisementId {
  ...
  static create(value: string): Result<AdvertisementId> {
    ...
    if (!result.success) {
      return { success: false, error: new Error('Invalid advertisement ID format') };
    }
...
export class CrowdFundingId {
  ...
  static create(value: string): Result<CrowdFundingId> {
    ...
    if (!result.success) {
      return { success: false, error: new Error('Invalid crowd funding ID format') };
    }
...
export class InvestmentId {
  ...
  static create(value: string): Result<InvestmentId> {
    ...
    if (!result.success) {
      return { success: false, error: new Error('Invalid investment ID format') };
    }
...
export class Location {
  ...
  static create(
    ...
    if (!address || address.trim().length < 10) {
      return { success: false, error: new Error('Address must be at least 10 characters') };
    }

    // 위도/경도 검증
    if (latitude < -90 || latitude > 90) {
      return { success: false, error: new Error('Latitude must be between -90 and 90') };
    }

    if (longitude < -180 || longitude > 180) {
      return { success: false, error: new Error('Longitude must be between -180 and 180') };
    }
...
export class QRCode {
  ...
  static create(merchantId: string, expiresAt: Date): Result<QRCode> {
    ...
    if (expiresAt <= new Date()) {
      return { success: false, error: new Error('Expiration time must be in the future') };
    }
...
export class Rating {
  ...
  static create(score: number, maxScore: number = 5): Result<Rating> {
    if (score < 0 || score > maxScore) {
      return { success: false, error: new Error(`Rating must be between 0 and ${maxScore}`) };
    }
  ...
  toString(): string {
    return `${this.score}/${this.maxScore} (${this.getPercentage().toFixed(1)}%)`;
  }
}
...
export class RewardRate {
  ...
  static createPercentage(rate: number): Result<RewardRate> {
    if (rate < 0 || rate > 100) {
      return { success: false, error: new Error('Percentage rate must be between 0 and 100') };
    }
  ...
  static createFixed(amount: number): Result<RewardRate> {
    if (amount < 0) {
      return { success: false, error: new Error('Fixed amount must be non-negative') };
    }
...
export class FundingGoal {
  ...
  static create(
    ...
    if (targetAmount <= 0) {
      return { success: false, error: new Error('Target amount must be positive') };
    }

    if (deadline <= new Date()) {
      return { success: false, error: new Error('Deadline must be in the future') };
    }
  ...
  equals(other: FundingGoal): boolean {
    return this.targetAmount === other.targetAmount &&
...
export class ViewingDuration {
  ...
  static create(totalSeconds: number, completionPercentage: number): Result<ViewingDuration> {
    if (totalSeconds < 0) {
      return { success: false, error: new Error('Total seconds must be non-negative') };
    }

    if (completionPercentage < 0 || completionPercentage > 100) {
      return { success: false, error: new Error('Completion percentage must be between 0 and 100') };
    }
  ...
  calculateReward(baseRewardPerMinute: number, bonusMultiplier: number = 1.5): number {
    const baseReward = this.getTotalMinutes() * baseRewardPerMinute;
    return this.isCompleteViewing() ? Math.round(baseReward * bonusMultiplier) : baseReward;
  }
  ...
  toString(): string {
    ...
    return `${minutes}:${seconds.toString().padStart(2, '0')} (${this.completionPercentage}%)`;
  }
}
