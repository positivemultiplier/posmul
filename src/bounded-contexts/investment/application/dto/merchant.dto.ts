...
export const CreateMerchantRequestSchema = z.object({
  ...
  businessHours: z.object({
    open: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, '올바른 시간 형식(HH:MM)을 입력해주세요'),
    close: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, '올바른 시간 형식(HH:MM)을 입력해주세요'),
    closedDays: z.array(z.string()).optional()
  }),
  rewardRate: z.number().min(0, '보상률은 0% 이상이어야 합니다').max(50, '보상률은 50% 이하여야 합니다')
});

export const UpdateMerchantRequestSchema = CreateMerchantRequestSchema.partial().extend({
