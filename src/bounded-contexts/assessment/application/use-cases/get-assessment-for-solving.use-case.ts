...
import { AssessmentForSolvingDto, QuestionForSolvingDto } from "../dto/assessment-for-solving.dto";
import { AssessmentId } from "../../domain/entities/assessment.entity";
import { Result, success, failure } from "@/shared/errors";
import { UseCaseError, NotFoundError } from "@/shared/errors";

export class GetAssessmentForSolvingUseCase {
  constructor(private readonly assessmentRepository: IAssessmentRepository) {}

  async execute(assessmentId: AssessmentId): Promise<Result<AssessmentForSolvingDto, UseCaseError | NotFoundError>> {
    try {
      const assessmentResult = await this.assessmentRepository.findById(assessmentId);
      if (!assessmentResult.success) {
        return failure(new UseCaseError("Failed to retrieve assessment.", assessmentResult.error));
      }

      const assessment = assessmentResult.data;
      if (!assessment) {
        return failure(new NotFoundError(`Assessment with id ${assessmentId} not found.`));
      }

      // Map domain entities to DTOs, stripping sensitive information
      const questionsForSolving: QuestionForSolvingDto[] = assessment.questions.map(q => ({
        id: q.id,
        type: q.type,
        title: q.title,
        content: q.content,
      ...
      const assessmentForSolving: AssessmentForSolvingDto = {
        ...
        questions: assessment.isRandomized ? this.shuffleArray(questionsForSolving) : questionsForSolving,
      ...
      return failure(new UseCaseError("An unexpected error occurred.", error as Error));
    }
  }
