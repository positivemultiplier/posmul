...
export class Institute {
  ...
  updateInfo(
    ...
    if (this.status === InstituteStatus.TERMINATED) {
      return { success: false, error: new Error('Cannot update terminated institute') };
    }

    if (!name || name.trim().length === 0) {
      return { success: false, error: new Error('Institute name cannot be empty') };
    }

    if (!description || description.trim().length === 0) {
      return { success: false, error: new Error('Institute description cannot be empty') };
    }
  ...
  approve(registrationNumber?: string): Result<void> {
    if (this.status !== InstituteStatus.PENDING) {
      return { success: false, error: new Error('Only pending institutes can be approved') };
    }
  ...
  suspend(_reason?: string): Result<void> {
    if (this.status !== InstituteStatus.ACTIVE) {
      return { success: false, error: new Error('Only active institutes can be suspended') };
    }
  ...
  reactivate(): Result<void> {
    if (this.status !== InstituteStatus.SUSPENDED) {
      return { success: false, error: new Error('Only suspended institutes can be reactivated') };
    }
  ...
  terminate(_reason?: string): Result<void> {
    if (this.status === InstituteStatus.TERMINATED) {
      return { success: false, error: new Error('Institute is already terminated') };
    }
  ...
  upgradeTrustLevel(newLevel: TrustLevel): Result<void> {
    const levelOrder = [TrustLevel.BASIC, TrustLevel.VERIFIED, TrustLevel.PREMIUM, TrustLevel.GOLD];
    const currentIndex = levelOrder.indexOf(this.trustLevel);
    const newIndex = levelOrder.indexOf(newLevel);

    if (newIndex <= currentIndex) {
      return { success: false, error: new Error('Trust level can only be upgraded') };
    }
  ...
  // 기관 통계 계산 (외부 데이터 필요)
  calculateStats(donations: { amount: number; donorId: UserId }[]): InstituteStats {
