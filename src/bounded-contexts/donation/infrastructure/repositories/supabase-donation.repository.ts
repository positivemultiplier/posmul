...
interface DonationRecord {
  ...
  opinion_leader_id: string | null;  metadata: Record<string, unknown>;
...
export class SupabaseDonationRepository implements IDonationRepository {
  ...
  async save(donation: Donation): Promise<Result<void>> {
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to save donation: ${error.message}`) };
      }
  ...
  async findById(id: DonationId): Promise<Result<Donation | null>> {
    try {
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
        return { success: false, error: new Error(`Failed to find donation: ${error.message}`) };
      }
  ...
  async findByDonorId(
    ...
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to find donations: ${error.message}`) };
      }
  ...
  async search(
    ...
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to search donations: ${error.message}`) };
      }
   ...
   * 기부 통계 조회
   */
  async getStats(donorId?: UserId): Promise<Result<{
    ...
    try {
      let query = this.supabase.from('donations').select('amount, category, type');
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to get stats: ${error.message}`) };
      }
      ...
      const donationsByCategory = data.reduce((acc, record) => {
        acc[record.category as DonationCategory] = (acc[record.category as DonationCategory] || 0) + 1;
        return acc;
      }, {} as Record<DonationCategory, number>);

      const donationsByType = data.reduce((acc, record) => {
        acc[record.type as DonationType] = (acc[record.type as DonationType] || 0) + 1;
        return acc;
   ...
   * 월별 통계 조회
   */
  async getMonthlyStats(year: number, donorId?: UserId): Promise<Result<Array<{
    month: number;
    totalDonations: number;
    totalAmount: number;
  }>>> {
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to get monthly stats: ${error.message}`) };
      }
   ...
   * 연별 통계 조회
   */
  async getYearlyStats(donorId?: UserId): Promise<Result<Array<{
    year: number;
    totalDonations: number;
    totalAmount: number;
  }>>> {
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to get yearly stats: ${error.message}`) };
      }

      const yearlyStatsMap = new Map<number, { totalDonations: number; totalAmount: number }>();

      data.forEach(record => {
        const year = new Date(record.created_at).getFullYear();
        const existing = yearlyStatsMap.get(year) || { totalDonations: 0, totalAmount: 0 };
      ...
      const yearlyStats = Array.from(yearlyStatsMap.entries()).map(([year, stats]) => ({
        year,
        ...stats
  ...
  async getDashboardSummary(donorId: UserId): Promise<Result<{
    ...
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to get dashboard summary: ${error.message}`) };
      }
      ...
      const favoriteCategory = Object.entries(categoryCount).reduce((a, b) => 
        categoryCount[a[0] as DonationCategory] > categoryCount[b[0] as DonationCategory] ? a : b
      )[0] as DonationCategory;

      // 올해 기부 총액
      const currentYear = new Date().getFullYear();
      const yearlyTotal = data
        .filter(record => new Date(record.created_at).getFullYear() === currentYear)
        .reduce((sum, record) => sum + record.amount, 0);

      // 월 평균 (올해 기준)
      const monthsElapsed = new Date().getMonth() + 1;
      const monthlyAverage = monthsElapsed > 0 ? yearlyTotal / monthsElapsed : 0;
  ...
  async delete(id: DonationId): Promise<Result<void>> {
    try {
      ...
      if (error) {
        return { success: false, error: new Error(`Failed to delete donation: ${error.message}`) };
      }
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async findByType(_type: DonationType, _pagination: PaginationParams): Promise<Result<PaginatedResult<Donation>>> {
    throw new Error('findByType method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async findByInstituteId(_instituteId: string, _pagination: PaginationParams): Promise<Result<PaginatedResult<Donation>>> {
    throw new Error('findByInstituteId method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async findByOpinionLeaderId(_opinionLeaderId: string, _pagination: PaginationParams): Promise<Result<PaginatedResult<Donation>>> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async findByDateRange(_startDate: Date, _endDate: Date, _pagination: PaginationParams): Promise<Result<PaginatedResult<Donation>>> {
    throw new Error('findByDateRange method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getTotalByDonor(_donorId: UserId, _startDate?: Date, _endDate?: Date): Promise<Result<number>> {
    throw new Error('getTotalByDonor method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getTopDonors(_limit: number, _startDate?: Date, _endDate?: Date): Promise<Result<Array<{ donorId: string; totalAmount: number; donationCount: number }>>> {
    throw new Error('getTopDonors method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getCategoryStats(_startDate?: Date, _endDate?: Date): Promise<Result<Record<DonationCategory, { count: number; totalAmount: number }>>> {
    throw new Error('getCategoryStats method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getRecentActivity(_donorId: UserId, _limit: number): Promise<Result<Donation[]>> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getCompletionRate(_donorId?: UserId, _startDate?: Date, _endDate?: Date): Promise<Result<number>> {
    throw new Error('getCompletionRate method not implemented yet');
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getAverageAmount(_donorId?: UserId, _category?: DonationCategory): Promise<Result<number>> {
