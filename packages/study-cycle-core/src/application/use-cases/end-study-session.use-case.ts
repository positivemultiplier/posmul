...
import { EndStudySessionRequest, EndStudySessionResponse } from '../dto/study-session.dto';
import { Result, success, failure, IUseCase } from '@/shared/types';
import { UseCaseError } from '@/shared/errors';
import { DomainError } from '@/shared/errors';

export class EndStudySessionUseCase implements IUseCase<EndStudySessionRequest, EndStudySessionResponse> {
  constructor(private readonly studySessionRepository: IStudySessionRepository) {}

  async execute(request: EndStudySessionRequest): Promise<Result<EndStudySessionResponse, UseCaseError>> {
    try {
      ...
      if (session.userId !== request.userId) {
        return failure(new UseCaseError('User is not authorized to end this session.'));
      }
      ...
      if (!endResult.success) {
        ...
        return failure(new UseCaseError(endResult.error.message, endResult.error));
      }

      // 4. Save the updated session
      const saveResult = await this.studySessionRepository.save(session);
      if (!saveResult.success) {
        return failure(new UseCaseError('Failed to save the ended session', saveResult.error));
      }
