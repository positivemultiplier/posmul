...
import { AnswerDto, SubmitAssessmentDto } from "../dto/submit-assessment.dto";
...
export class SubmitAssessmentUseCase {
  ...
  async execute(
    ...
    Result<SubmissionResultDto, UseCaseError | NotFoundError | DomainError>
  > {
    try {
      ...
      if (!assessmentResult.success || !assessmentResult.data) {
        return failure(
          new NotFoundError(`Assessment with id ${dto.assessmentId} not found.`)
        );
      }
      ...
      if (!submissionResult.success) {
        return failure(
          new DomainError(
            "Failed to create submission entity.",
      ...
      const saveResult =
        await this.assessmentRepository.saveSubmission(submission);
      if (!saveResult.success) {
        return failure(
          new UseCaseError("Failed to save submission.", saveResult.error)
        );
      }
    ...
    switch (question.type) {
      ...
      case "multiple-select":
        ...
        return (
          ...
          Array.from(correctAnswerSet).every((item) => userAnswerSet.has(item))
        );
      case "short-answer":
