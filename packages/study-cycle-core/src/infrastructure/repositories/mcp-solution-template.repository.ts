...
export class McpSolutionTemplateRepository
  implements ISolutionTemplateRepository
{
  ...
  async findAll(): Promise<Result<SolutionTemplate[], RepositoryError>> {
    try {
      ...
      const templates = (result.data as SolutionTemplateRow[]).map((row) =>
        this.mapRowToTemplate(row)
      );
      return success(templates);
    } catch (error) {
      return failure(
        new RepositoryError("전체 SolutionTemplate 조회 실패", error)
      );
    }
  }
  ...
  async findByType(
    ...
    try {
      ...
      return failure(
        new RepositoryError("타입별 SolutionTemplate 조회 실패", error)
      );
    }
  }

  async findActiveTemplates(): Promise<
  ...
  > {
    try {
      ...
      const result = await mcp_supabase_execute_sql({
        project_id: this.projectId,
        query:
          "SELECT * FROM solution_templates WHERE is_active = true ORDER BY created_at DESC",
      });
      ...
      return failure(
        new RepositoryError("활성 SolutionTemplate 조회 실패", error)
      );
    }
  }
  ...
  async findByTitle(
    ...
    try {
      ...
      return failure(
        new RepositoryError("제목별 SolutionTemplate 조회 실패", error)
      );
    }
  }

  async findByTitleContaining(
    searchTerm: string
  ): Promise<Result<SolutionTemplate[], RepositoryError>> {
    try {
      const { mcp_supabase_execute_sql } = await import("@posmul/shared-auth");
      ...
      return failure(
        new RepositoryError("제목 검색 SolutionTemplate 조회 실패", error)
      );
    }
  }
  ...
  async getTemplateUsageStats(
    ...
    try {
      ...
      const stats: TemplateUsageStats = {
        ...
        lastUsedAt: statsRow.last_used_at
          ? new Date(statsRow.last_used_at)
          : null,
  ...
  async getMostUsedTemplates(
    ...
    try {
      ...
      return failure(
        new RepositoryError("인기 SolutionTemplate 조회 실패", error)
      );
    }
  }
