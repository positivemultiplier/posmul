 ...
 * 성공 결과 생성 헬퍼
 */
export function success<T, E = UniversalError>(
  data: T,
  metadata?: ResultMetadata
): UniversalResult<T, E> {
 ...
 * 실패 결과 생성 헬퍼
 */
export function failure<T, E = UniversalError>(
  error: E,
  metadata?: ResultMetadata
): UniversalResult<T, E> {
  if (metadata !== undefined) {
 ...
 * 타입 가드: 성공 결과인지 확인
 */
export function isSuccess<T, E = UniversalError>(
  result: UniversalResult<T, E>
 ...
 * 타입 가드: 실패 결과인지 확인
 */
export function isFailure<T, E = UniversalError>(
  result: UniversalResult<T, E>
