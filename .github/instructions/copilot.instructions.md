...
### Database Schema Creation (With Monorepo Context)

```typescript
// Use MCP for all schema operations
// File: apps/posmul-web/src/shared/mcp/migrations.ts
export const createPredictionTables = async (projectId: string) => {
...
### Repository Implementation with MCP (Monorepo)

```typescript
// File: apps/posmul-web/src/bounded-contexts/prediction/infrastructure/repositories/mcp-prediction-game.repository.ts
import { Result } from 'shared-types';
import { IPredictionGameRepository } from '../../domain/repositories/prediction-game.repository';
import { PredictionGame } from '../../domain/entities/prediction-game.entity';
...
## TypeScript Standards (Monorepo + MCP)
...
### Result/Either pattern in shared-types:
...
## CRITICAL REMINDERS (Monorepo Edition)
...
5. **Respect package boundaries** - don't directly import across packages
6. **Use PowerShell commands** - never bash/zsh
7. **Always implement Result pattern** for error handling
8. **Repository interfaces in Domain** - implementations in Infrastructure
9. **Server Components by default** - Client only when needed
10. **🔥 USE MCP TOOLS FOR ALL DATABASE AND PROJECT OPERATIONS**
11. **🔥 ECONOMIC INTEGRATION IS CRITICAL** - All domains must integrate with PMP/PMC system
12. **Use Domain Events for cross-domain economic transactions**
13. **Shared Economy Kernel for read access, Events for write access**
14. **Always display economic balance in relevant UI components**
15. **🔥 ALWAYS run security advisors after schema changes**
...
## Monorepo + Dual MCP Workflow Integration

### Complete Development Cycle

1. **Issue Creation** → `mcp_Github_create_issue` (with monorepo context)
2. **Package Development** → Use turbo for builds
3. **Database Design** → `mcp_supabase_apply_migration`
4. **Cross-Package Integration** → Use workspace protocols
5. **Testing** → `turbo test` (all packages)
6. **Pull Request** → `mcp_Github_create_pull_request`
7. **Security Check** → `mcp_supabase_get_advisors`
8. **Type Generation** → `mcp_supabase_generate_typescript_types`

### Quality Assurance Integration

- **GitHub MCP**: DDD architecture compliance, PowerShell compatibility
- **Supabase MCP**: RLS policies, performance optimization
- **Turbo**: Build optimization, cache management
- **pnpm**: Dependency management, workspace integrity
- **Economic System**: Agency Theory logic verification, PMP/PMC integrity

When in doubt, ask yourself: "Does this follow DDD principles, Clean Architecture rules, respect monorepo boundaries, use pnpm + turbo, use DUAL MCP tools (Supabase + GitHub), AND properly integrate with the economic system?"
